...
mixin MapControllerMixin<T extends StatefulWidget> on State<T> {
  ...
  /// Settle the camera when the driver arrives.
  ///
  /// Navigation framing (tilted, course-up, zoomed to the road ahead) is for
  /// driving. On arrival Waze and Google both flatten the map to north-up
  /// and zoom in on the destination, so you can see which side of the street
  /// you're on and where the entrance is.
  Future<void> settleCameraOnArrival() async {
    // Hand the camera back from the native viewport first, or it keeps
    // driving and immediately undoes this.
    exitNativeViewport();
    _cameraController.disableNavigationMode();
